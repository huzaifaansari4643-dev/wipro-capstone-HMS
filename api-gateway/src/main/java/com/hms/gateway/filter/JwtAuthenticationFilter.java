...
import com.hms.gateway.utils.JwtUtil;
...
public class JwtAuthenticationFilter extends AbstractGatewayFilterFactory<JwtAuthenticationFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (config.isRequiresRole()) {
                ...
                if (!isAuthorized(userRole, config)) {
                    System.out.println("User role " + userRole + " not authorized");
    ...
    private boolean isAuthorized(String userRole, Config config) {
        if (userRole.equals("ADMIN")) {
            return true;
        }
        
        if (config.getRequiredRoles() == null || config.getRequiredRoles().isEmpty()) {
            return true; // Allow any authenticated if no specific roles set
        }
